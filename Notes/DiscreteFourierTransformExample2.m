...
for i1 = 1:length(f)
    ...
    X(i1) = s/Fs; % divide by Fs to replicate dt (infintesimaly small period of time)
end
...
for i1 = 1:length(n2)
    ...
    for i2 = (1:length(range))+floor(length(f2)/2)
        s = s + Xs(i2)*exp(j*f2(i2)*i1);
    end
    xsr(i1) = s/2/pi/Fs; % divide by Fs to replciate inifntesimaly small dt
end
